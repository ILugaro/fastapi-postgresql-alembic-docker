...
async def test_create_products_from_file():
    ...
    products: list[Product] = await ParserService.create_products_from_excel(str(os.path.join(folder_path, 'test_data', 'test_detskiymir_msk_products.xlsx')))
    ...
    assert products[41146].category_path.category_path == 'Главная::Детские игрушки::Настольные игры::Развивающие::Развивающие Djeco'
...
async def test_get_product_by_id(ac: AsyncClient):
    ...
    assert product['category_path'] == {'category_path':'Главная::Детские игрушки::Детские конструкторы::Блочные конструкторы::Блочные SLUBAN', 'id': 1}
...
async def test_get_product_by_id(ac: AsyncClient):
    """Проверка продукта с пустыим не обязательными атрибутами"""
    ...
    assert product['category_path'] == {'category_path':'Главная::Детские игрушки::Настольные игры::Развивающие::Развивающие Djeco', 'id': 3}
